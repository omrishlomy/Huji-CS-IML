...
def train_xgb():
    ...
    X_train, y_train = training_data,training_labels
    models = []
    accuracies = []
    n_estimators = [1,5,15,25]
    for estimator in n_estimators:
        # Initialize the XGBClassifier
        model = XGBClassifier(n_estimators=estimator, max_depth=1, learning_rate=0.1)
    ...
    return models,n_estimators,accuracies

if __name__ == '__main__':
    # decision_tree_demo()
    # K_1 = KNNClassifier(1, 'l2')
    # K_1.fit(training_data, training_labels)
    # K_2 = KNNClassifier(3000, 'l2')
    # K_2.fit(training_data, training_labels)
    # K_3 = KNNClassifier(1, 'l1')
    # K_3.fit(training_data, training_labels)
    # models = [K_1, K_2, K_3]
    # for model in models:
    #     plot_decision_boundaries(model, testing_data, testing_labels,"Decision Boundaries" + " " + str(model.k) + " " + model.distance_metric)

    xgb_models,n_estimators,accuracies = train_xgb()
    i=0
    ...
    for model in xgb_models:
        plot_decision_boundaries(model, testing_data, testing_labels,"Decision Boundaries" + str(n_estimators[i]))
        i+=1
